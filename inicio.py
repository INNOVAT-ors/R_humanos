...
def ed_tipo_nom(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select count(*) from percepcion where id_nomina= {0}'.format (id))
...
def ed_periodo(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select count(*) from per_puesto where idperiodo= {0}'.format (id))
...
def modifica_periodo(id):
    if request.method == 'POST':
        ...
        if (per[0] != 0):
            ...
            cursor.execute('update periodo set  descripcion=%s where idperiodo=%s', (aux_descripcion, id))
            conn.commit()
            conn.close()
            return redirect(url_for('periodo'))
...
def bo_percepciones(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select count(*) from per_puesto where idpercepcion = {0}'.format(id))
    solicitudes = cursor.fetchone()
    if (solicitudes[0] != 0):
        error = "La percepción tiene dependientes, no puede ser borrado."
